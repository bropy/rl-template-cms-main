  ...
  if (likedBooks.length === 0) {
    ...
    return (
      <section className={cn('w-full', className)}>
        ...
        <Card className='border-divider border-1'>
          <CardBody className='p-8'>
            <div className='py-8 text-center'>
              ...
              <p className='text-foreground/60 text-lg'>Start exploring and like some books to see them here!</p>
            </div>
          </CardBody>
        </Card>
      </section>
        ...
        <CardBody className='p-6'>
            ...
            {displayedBooks.map((book) => {
              ...
              return (
                <div key={book.key} className='relative'>
                  <BookCardComponent book={bookForCard} compact={true} />
                  <div className='text-foreground/50 bg-background/80 absolute top-2 left-2 rounded px-2 py-1 text-xs backdrop-blur-sm'>
          ...
          {hasMoreBooks && (
            <div className='mt-6 text-center'>
              <Button variant='bordered' size='sm' onPress={() => setIsExpanded(!isExpanded)}>
                {isExpanded ? <>↑ Show Less</> : <>↓ Show {likedBooks.length - 3} More Books</>}
              </Button>
            </div>
          )}
          ...
          {likedBooks.length > 0 && (
            <div className='border-divider mt-6 border-t pt-4'>
              <div className='text-foreground/50 flex items-center justify-between text-xs'>
                <span>Total liked books: {likedBooks.length}</span>
                <span>Latest: {new Date(likedBooks[0]?.likedAt || '').toLocaleDateString()}</span>
              </div>
            </div>
          )}
        </CardBody>
